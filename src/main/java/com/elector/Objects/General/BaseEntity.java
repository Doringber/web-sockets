...
public abstract class BaseEntity {
    ...
    public void setObject(BaseEntity other){
        setObject(other, true);
    }

    public void setObject(BaseEntity other, boolean includeCurrentClassFields){
        try {
            Field[] fields = new Field[0];
            Class clazz = this.getClass();
            do {
                if (includeCurrentClassFields) {
                    fields =(Field[]) ArrayUtils.addAll(fields, clazz.getDeclaredFields());
                }
                includeCurrentClassFields = true;
